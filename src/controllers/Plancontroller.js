...
export const generatePlan = async (req, res) => {
  const { career, level, time, weeks } = req.body;

  if (!career || !level || !time || !weeks) {
    return res.status(400).json({ error: "Missing input fields" });
  }

  try {
    const prompt = `Create a personalized :${weeks}-week study plan for a student.
Career Goal: ${career}
Level: ${level}
...
The plan should be suitable for a beginner in the field of ${career} at the ${level} level.
